...
def get_random_valid_xy(ascii_val, ima, xsize, ysize):
    ...
    rgb = ima[x_ran, y_ran]
    if rgb[0] + ascii_val <= 255: #or rgb[1] + ascii_val <= 255 or rgb[2] + ascii_val <= 255:
        return x_ran, y_ran
...
for letter in text:
    ...
    x, y = get_random_valid_xy(ascii_value, image, x_size, y_size)  # Random position
    print("x:", x, " y:", y)
    RGB = image[x, y]
